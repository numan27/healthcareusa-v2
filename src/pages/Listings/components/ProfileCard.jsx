    ...
    useEffect(() => {
        const fetchPosts = async () => {
            ...
            try {
                ...
                // Fetch media URLs
                const mediaIds = response.data.map(post => post.featured_media).filter(id => id);
                const mediaResponses = await Promise.all(mediaIds.map(id => axios.get(`https://jsappone.demowp.io/wp-json/wp/v2/media/${id}`)));
    ...
    return (
        <>
            {profile.map((data) => {
                ...
                const designation = cubewpPostMeta["cwp_field_40228862441"]?.meta_value || '';
                    ...
                    <Box
                                ...
                                <Row>
                                    ...
                                    <Col className={`d-flex flex-column justify-content-between ${columnPadding ? 'ps-1' : ''}`}>
                                        <div className='d-flex flex-sm-row flex-column justify-content-between align-items-sm-center'>
                                            ...
                                            {enableSponsoredProfile && (
                                                <img className="mt-4" width={100} src={IMAGES.PROFILE_COMPANY_LOGO} alt="" />
                                            )}
                                        </div>
                                        ...
                                        {enableSponsoredProfile && (
                                            <div className='d-flex align-items-end flex-wrap flex-sm-nowrap gap-2 mt-2xl-0 mt-3'>
                                                <GenericButton
                                                    borderColor="transparent"
                                                    background={data.comment_status === 'Close' ? '#23262F' : '#00C1B6'}
                                                    hoverBgColor={data.comment_status === 'Close' ? '#23262F' : '#00ADA2'}
                        ...
                        {enableSponsoredProfile && (
                            <div
                                ...
                                <Typography as="p" className="mb-0 text-capitalize" color="#23262F" size="14px" lineHeight="19px" weight="500">
                                    {data.comment_status}
                                </Typography>
                            </div>
                        )}
