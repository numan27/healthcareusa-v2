    ...
    const fetchMenu = async () => {
      try {
        const response = await axios.get(
          "https://jsappone.demowp.io/wp-json/wp/v2/widgets/",
          {
            auth: {
              username: "numan27",
        ...
        const extractedLinks = widgets
          .map((widget) => {
            if (widget.rendered) {
              const parser = new DOMParser();
              const doc = parser.parseFromString(widget.rendered, "text/html");
              const heading =
                doc.querySelector(".widget-title")?.textContent || widget.id;
        ...
        <Row>
          <Col sm={9} md={10} xs={11} className="mx-sm-auto mx-4">
            <Row className="py-4 border-bottom">
              <Col
                lg={6}
                className="px-lg-0 d-flex align-items-center justify-content-center justify-content-lg-start mb-4 mb-lg-0"
              >
              ...
              <Col
                lg={6}
                className="d-flex flex-lg-row flex-column gap-4 justify-content-lg-end justify-content-center align-items-center px-lg-0"
              >
            ...
            <Row className="mt-4 py-3 border-bottom">
              <Col className="ps-0" lg={3}>
                ...
                <Typography
                  ...
                  color="#333333"
                >
                  Lorem ipsum dolor sit amet consectetur. Elit netus neque
                  faucibus pharetra ut. At sit lobortis ni
                </Typography>
                        ...
                        <div key={index} className="mb-sm-0 mb-3">
                          ...
                          <ul className="list-unstyled footer-list">
                            {group.items.map((item, itemIndex) => (
                              <li key={itemIndex}>
                                <Link className="transition-2" to={item.link}>
                                  {item.name}
                                </Link>
                              </li>
                            ))}
            ...
            <Row className="my-3 d-flex flex-lg-row flex-column-reverse justify-content-between">
              ...
              <Col
                xl={4}
                lg={6}
                className="d-flex justify-content-lg-end justify-content-center px-0"
              >
